...
const App = () => {
  ...
  const awardTokens = async (walletAddress, amount, reason) => {
    try {
      await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5002'}/api/blockchain/award-tokens`, {
        method: 'POST',
  ...
  const createNFT = async (walletAddress, description) => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5002'}/api/blockchain/create-nft`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const fetchAIMatches = useCallback(async () => {
    if (!publicKey) return;
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5002'}/api/ai/matches/${publicKey.toString()}`);
  ...
  const fetchChatAnalytics = useCallback(async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5002'}/api/ai/analytics`);
      const data = await response.json();
      if (data.success) {
  ...
  const sendMessage = async () => {
    ...
    if (input.toLowerCase().startsWith('/ai ')) {
      ...
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5002'}/api/ai/chat`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        const aiMessage = {
          id: Date.now() + 1,
          sender: 'SYSTEM',
          content: `❌ AI service unavailable. Make sure backend is running on localhost:5002`,
          timestamp: new Date(),
          type: 'error'
        };
